...
def format_group_test_stats_title(
    *,
    ...
    return ','.join([
        ...
        f'\n Implied AnnuFee({fee_bps:g} bps): {implied_fee:.2%}, Daily IC: {rank_ic:.4f}, Annu ICIR: {icir:.2f}',
    ])
...
# %% Mentor single-factor protocol helpers
# Convention: G1 = short (lowest factor), G10 = long (highest factor), H-L = G10 - G1.
# Headline excess Sharpe = G10 − exact valid-universe equal-weight (not mean of deciles).
EXCESS_SHARPE_GATE = 3.5
...
def apply_tradability_mask(signal, ret=None, startDay=None, endDay=None,
                           not_limit=None, not_st=None):
    """
    Drop ST / limit-up-down names from signal (and optional ret) via 1/NaN masks.

    Usage:
        signal_c, ret_c = apply_tradability_mask(signal, ret)
    """
    if startDay is None:
        startDay = signal.index[0]
    if endDay is None:
    ...
    if not_limit is None:
        not_limit = get_EOD_Not_Limit(startDay, endDay)
    if not_st is None:
        not_st = get_EOD_Not_ST(startDay, endDay)

    not_limit = not_limit.reindex_like(signal)
    not_st = not_st.reindex_like(signal)
    tradable = not_limit.notna() & not_st.notna()

    signal_out = signal.where(tradable)
    if ret is None:
        return signal_out
    ret_out = ret.reindex_like(signal).where(tradable)
    return signal_out, ret_out


def get_index_member_mask(index_code, startDay, endDay):
    """
    Daily index membership wide mask: 1 = in index, NaN = out.

    HS300 / CSI500 / CSI1000 use Wind daily weight tables (TRADE_DT rows).
    Other indices expand AINDEXMEMBERS in/out dates onto trading days
    (same approach as intraday_lib.get_stock_pool).
    """
    s = ddb.session()
    s.connect(**DATA_DB_CONN)
    start_str = pd.Timestamp(startDay).strftime('%Y.%m.%d')
    end_str = pd.Timestamp(endDay).strftime('%Y.%m.%d')
    ...
    try:
        if index_code in weight_db:
            t = s.loadTable(dbPath=weight_db[index_code], tableName='data')
            df = t.select('TRADE_DT, S_CON_WINDCODE').where(
                f"TRADE_DT >= {start_str} and TRADE_DT <= {end_str}"
            ).toDF()
            if df.empty:
                s.close()
            ...
            df['flag'] = 1.0
            wide = df.pivot_table(
                index='TRADE_DT', columns='S_CON_WINDCODE', values='flag', aggfunc='last'
            )
        ...
        script = (
            ...
            "index_members = select S_CON_WINDCODE,\n"
            "    S_CON_INDATE,\n"
            "    nullFill(temporalParse(S_CON_OUTDATE, \"yyyyMMdd\"), 2100.01.01) as S_CON_OUTDATE\n"
            "    from t\n"
        ...
        members = s.run(script)
        s.close()
        if members is None or len(members) == 0:
            return pd.DataFrame(index=tradingDay, dtype=float)

        members = pd.DataFrame(members)
        members['S_CON_INDATE'] = pd.to_datetime(members['S_CON_INDATE'])
        members['S_CON_OUTDATE'] = pd.to_datetime(members['S_CON_OUTDATE'])
        trade_arr = tradingDay.to_numpy(dtype='datetime64[ns]')
        left = np.searchsorted(
            trade_arr, members['S_CON_INDATE'].to_numpy(dtype='datetime64[ns]'), side='left'
        )
        right = np.searchsorted(
            trade_arr, members['S_CON_OUTDATE'].to_numpy(dtype='datetime64[ns]'), side='right'
        )
        parts = []
        for code, lo, hi in zip(members['S_CON_WINDCODE'].to_numpy(), left, right):
            if lo >= hi:
                continue
            parts.append(pd.DataFrame({
                'TRADE_DT': tradingDay[lo:hi],
                'S_CON_WINDCODE': code,
                'flag': 1.0,
            }))
        if not parts:
        ...
        return wide.reindex(tradingDay).sort_index()
    except Exception:
        s.close()
        raise


def calc_forward_returns(ret, periods=(1, 5, 10, 20)):
    """
    N-day compound returns aligned for use with ``signal.shift(1)``.

    Value at date T = (1+r_T)*...*(1+r_{T+N-1}) - 1.
    Together with ``align_signal(..., shift=1)``, factor on T-1 predicts the next N days
    starting at T (same convention as 1-day ``groupTest``).

    Vectorized via log1p / expm1 (no rolling.apply).
    """
    log_ret = np.log1p(ret.astype(float))
    out = {}
    for p in periods:
        # rolling(p) at T+p-1 covers T .. T+p-1; shift(-(p-1)) moves that block to T.
        fwd_log = log_ret.rolling(window=p, min_periods=p).sum()
        if p > 1:
            fwd_log = fwd_log.shift(-(p - 1))
        out[int(p)] = np.expm1(fwd_log)
    return out


def summarize_group_sharpes(group_pnl_df, n=250):
    """Sharpe for G1..Gn and H-L from groupTest pnl columns."""
    rows = {}
    for col in group_pnl_df.columns:
        rows[col] = calSharpe(group_pnl_df[col], n=n)
    return pd.Series(rows, name='sharpe')


def g10_excess_vs_universe_ew(signal, ret, *, signal_shift=1, top_frac=0.10, n=250):
    """
    G10 (highest factor) equal-weight return minus exact valid-universe EW.

    Mentor convention: G1=short, G10=long. Flip negative-IC factors to Alpha first
    so G10 remains the long book. Benchmark is mean of stocks with both valid
    aligned signal and return — not the mean of decile pnls.
    """
    sig = signal.shift(signal_shift) if signal_shift else signal
    r = ret.reindex_like(sig)
    valid = sig.notna() & r.notna()
    ranks = sig.rank(axis=1, pct=True, method='first')
    g10_mask = valid & (ranks > 1.0 - top_frac)
    long_ret = r.where(g10_mask).mean(axis=1)
    universe_ew = r.where(valid).mean(axis=1)
    excess = long_ret - universe_ew
    return {
        'long_group': 10,
        'excess_sharpe': calSharpe(excess, n=n),
        'excess_annu_ret': calAnnuRet(excess, n=n),
        'excess_max_drawdown': calMDD(excess)[0],
        'selected_count_mean': float(g10_mask.sum(axis=1).mean()),
        'universe_count_mean': float(valid.sum(axis=1).mean()),
        'n_days': int(excess.dropna().shape[0]),
        '_long_ret': long_ret,
        '_universe_ew_ret': universe_ew,
        '_excess_ret': excess,
    }


def check_excess_gates(excess_sharpe, group_sharpes, gate=EXCESS_SHARPE_GATE):
    """
    Mentor gates: excess Sharpe > gate and > every G1..G10 / H-L Sharpe.
    """
    gs = pd.Series(group_sharpes).dropna()
    max_group = float(gs.max()) if len(gs) else np.nan
    ex = float(excess_sharpe) if excess_sharpe is not None else np.nan
    return {
        'excess_sharpe': ex,
        'max_group_or_hl_sharpe': max_group,
        'gate': float(gate),
        'pass_gt_gate': bool(ex > gate) if np.isfinite(ex) else False,
        'pass_gt_all_group_sharpes': (
            bool(ex > max_group) if (np.isfinite(ex) and np.isfinite(max_group)) else False
        ),
        'pass_all': bool(np.isfinite(ex) and ex > gate and ex > max_group),
    }


def restrict_to_index(signal, ret, index_mask):
    """Keep only index members (index_mask == 1 / notna)."""
    m = index_mask.reindex_like(signal)
    keep = m.notna() & (m == 1)
    return signal.where(keep), ret.reindex_like(signal).where(keep)
...
if __name__ == '__main__':
    ...
    _ = get_EOD_Not_ST(startDay, endDay)
    _ = get_EOD_Not_Limit(startDay, endDay)

#%%
"""
Mentor single-factor helpers (resolves prior TODOs):
1. ST / limit filter → apply_tradability_mask
2. Index universes → get_index_member_mask + restrict_to_index
3. Decay → calc_forward_returns + groupTest
4. G10 excess Sharpe → g10_excess_vs_universe_ew + check_excess_gates
"""
