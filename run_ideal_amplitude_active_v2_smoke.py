#!/usr/bin/env python
"""IdealAmplitude_ActiveV2 smoke — one month amp/netvol panel.

Usage:
  OMP_NUM_THREADS=1 python run_ideal_amplitude_active_v2_smoke.py
  OMP_NUM_THREADS=1 python run_ideal_amplitude_active_v2_smoke.py --year 2024 --month 6
...
from factor_formulas_ideal_amplitude_active_v2 import process_factor_cross_section

REPO = Path(__file__).resolve().parent
SMOKE_OUT = REPO / "research/reports/ideal_amplitude_active_v2/smoke"
...
def main() -> None:
    ...
    log("=== IdealAmplitude_ActiveV2 SMOKE ===")
    log(f"formula={FORMULA_VERSION}")
    log("identity=realized_amp / active_net_vol EWM5 (NOT IdealAmplitude price×amp)")
    ...
    assert "active_buy_amt" in raw_cols and "amount" in raw_cols, raw_cols
    ...
    log("L3 factor panel ...")
    wide, long = build_ideal_amplitude_panel(
        start,
        end,
        use_cache=True,
    ...
    log(
        f"  amp_smooth describe: mean={dist.get('mean')} p50={dist.get('p50')} "
        f"std={dist.get('std')}"
    )
    if not long.empty and "amp_raw" in long.columns:
        raw_vals = long["amp_raw"].to_numpy(dtype=float)
        raw_vals = raw_vals[np.isfinite(raw_vals)]
        log(
            f"  amp_raw n={len(raw_vals)} "
            f"mean={float(np.mean(raw_vals)) if len(raw_vals) else None} "
            f"p50={float(np.median(raw_vals)) if len(raw_vals) else None}"
        )
    ...
    report = {
        ...
        "identity": "IdealAmplitude_ActiveV2",
        "not_ideal_amplitude_price": True,
        "use_gate": args.use_gate,
    }
