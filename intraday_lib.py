...
def _group_display_name(group) -> str:
    """DDB ``group_0``…``group_9`` / ``group_HML`` → 日频标准标签 ``1``…``10`` / ``H-L``."""
    g = str(group)
    if "HML" in g.upper() or g in ("H-L", "HL"):
        return "H-L"
...
def _group_plot_order(display_names):
    def _key(name):
        if name == "H-L":
            return (1, 0)
        try:
    ...
    return sorted(display_names, key=_key)


def _lookup_ic_stats(ic_mean, bartime, ret_col):
    """从 DDB ``summary['ic_mean']`` 取 Daily IC / Annu ICIR；缺失时返回 nan。"""
    if ic_mean is None or (isinstance(ic_mean, pd.DataFrame) and ic_mean.empty):
        return float("nan"), float("nan")
    df = ic_mean.copy()
    # 列名可能是 RetType / valueType
    ret_key = "RetType" if "RetType" in df.columns else (
        "valueType" if "valueType" in df.columns else None
    )
    if ret_key is None or "Bartime" not in df.columns:
        return float("nan"), float("nan")

    bt = pd.to_datetime(bartime)
    # Bartime 可能是 time / timedelta / datetime
    def _bt_match(series):
        s = pd.to_datetime(series.astype(str), errors="coerce")
        if s.notna().any():
            return s.dt.strftime("%H:%M:%S") == bt.strftime("%H:%M:%S")
        # fallback: string equality
        target = bt.strftime("%H:%M:%S")
        return series.astype(str).str.contains(target[:5], regex=False)

    mask = (df[ret_key].astype(str) == str(ret_col)) & _bt_match(df["Bartime"])
    sub = df.loc[mask]
    if sub.empty:
        # 单 Bartime 时只按 RetType 匹配
        sub = df.loc[df[ret_key].astype(str) == str(ret_col)]
    if sub.empty:
        return float("nan"), float("nan")
    row = sub.iloc[0]
    ic = float(row["IC_Mean"]) if "IC_Mean" in row.index and pd.notna(row["IC_Mean"]) else float("nan")
    # DDB IC_IR 通常未年化；日频图用 Annu ICIR = IC_IR * sqrt(250)
    if "IC_IR" in row.index and pd.notna(row["IC_IR"]):
        icir = float(row["IC_IR"]) * np.sqrt(250)
    else:
        icir = float("nan")
    return ic, icir


def _pivot_group_pnl(bartime_data, ret_col) -> pd.DataFrame:
    """窄表 → Date × {1..N, H-L} 宽表（与 Factor_Dev_Lib.groupTest 列名一致）。"""
    rows = []
    for group, gdf in bartime_data.groupby("group", sort=False):
        label = _group_display_name(group)
        s = gdf.sort_values("Date").set_index("Date")[ret_col]
        s.name = label
        rows.append(s)
    if not rows:
        return pd.DataFrame()
    pnl = pd.concat(rows, axis=1)
    # 同名列取首列（防御性）
    pnl = pnl.loc[:, ~pnl.columns.duplicated()]
    ordered = _group_plot_order([c for c in pnl.columns])
    return pnl.reindex(columns=ordered)


def intraday_turnover_b_hl(side_gross: float = None) -> float:
    """口径B：固定 Bartime 开仓 + 持有到期平仓的 H-L 日换手。

    与 H-L 收益定义一致（多空各 ``side_gross`` 倍权益）：
    ``Turnover_B = 2 × (long_gross + short_gross) / equity = 4 × side_gross``。
    """
    if side_gross is None:
        try:
            import factor_config as cfg

            side_gross = float(getattr(cfg, "INTRADAY_HL_SIDE_GROSS", 1.0))
        except Exception:  # noqa: BLE001
            side_gross = 1.0
    return 4.0 * float(side_gross)


def _blank_axes(ax, fig=None):
    """Research 风格：白底、无灰底、无 seaborn 网格底色。"""
...
def save_decile_hl_cumsum_png(
    ...
    *,
    ...
    """与 ``Factor_Dev_Lib.groupTest`` / ``save_group_cumsum_png`` 同风格的十分组+H-L 累计图。"""
    import Factor_Dev_Lib as fdl

    if group_pnl_df.empty or "H-L" not in group_pnl_df.columns:
        return None

    if avg_turnover is None:
        avg_turnover = intraday_turnover_b_hl()

    hl = group_pnl_df["H-L"].dropna()
    if hl.empty:
        return None
    ...
    implied_fee = fdl.implied_annu_fee(avg_turnover)
    stats_title = fdl.format_group_test_stats_title(
        ...
        rank_ic=rank_ic if pd.notna(rank_ic) else float("nan"),
        icir=icir if pd.notna(icir) else float("nan"),
        implied_fee=implied_fee,
    )

    cum_pnl_df = group_pnl_df.fillna(0).cumsum()
...
def save_decile_hl_mean_bar_png(group_pnl_df: pd.DataFrame, *, title: str, out_path: str):
    """分组平均收益柱状图（含 H-L），与日频 ``group_mean_pnl.png`` 同风格。"""
    if group_pnl_df.empty:
        return
    fig, ax = plt.subplots(figsize=(10, 6))
    _blank_axes(ax, fig)
    group_pnl_df.mean().plot(kind="bar", ax=ax, title=title)
    _blank_axes(ax, fig)
    fig.tight_layout()
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    fig.savefig(out_path, dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
...
def analyze_group_performance_by_bartime(
    group_data_ret,
    ret_columns=['Ret_15', 'Ret_30', 'Ret_60', 'Ret_90','Ret_120', 'Ret_180','Ret_NDay'],
    save_plots=True,
    show_plots=False,
    save_path='result',
    factor_name: str = "",
    ...
    """
    按Bartime分类分析每个group的表现，并按日频十分组 H-L 标准格式出图。

    标签约定与 ``Factor_Dev_Lib.groupTest`` 一致：G1=最低组，G10=最高组，H-L=G10−G1。
    累计图：``figsize=(20,12)``，线末标注组别，legend 左上，xlabel 为标准统计页脚。
    另存分组均值柱状图 ``group_mean_{ret}.png``。

    Parameters
    ----------
    ic_mean : DataFrame, optional
        DDB ``summary['ic_mean']``，用于页脚 Daily IC / Annu ICIR。
    factor_name : str
        图标题前缀。
    """
    ...
    # 与 research / factor_runner 一致：白底 blank，不用 seaborn 灰底
    ...
    setup_chinese_font()
    avg_turnover_b = intraday_turnover_b_hl()

    raw_groups = [g for g in data['group'].unique() if str(g).startswith('group_')]
    # 数值组在前，HML 在后
    groups = sorted(
        raw_groups,
        key=lambda g: (0, int(str(g).split('_', 1)[1]))
        if str(g).startswith('group_') and str(g).split('_', 1)[1].isdigit()
        else (1, 0),
    )
    ...
    results_summary = {}
    multi_bt = len(bartimes) > 1
    fname_prefix = factor_name.strip() or "Factor"
    ...
    for ret_col in ret_columns:
        ...
        for bartime in bartimes:
            ...
            pnl = _pivot_group_pnl(bartime_data, ret_col)
            if pnl.empty or "H-L" not in pnl.columns:
                print(f"  skip plot {ret_col}@{bartime}: no H-L panel")
                continue

            rank_ic, icir = _lookup_ic_stats(ic_mean, bartime, ret_col)
            bt_tag = pd.to_datetime(bartime).strftime("%H%M")
            if multi_bt:
                cum_path = f'{save_path}/group_performance_{ret_col.lower()}_{bt_tag}.png'
                mean_path = f'{save_path}/group_mean_{ret_col.lower()}_{bt_tag}.png'
                plot_title = f'{fname_prefix} @ {pd.to_datetime(bartime).strftime("%H:%M")} — {ret_col} (G1-G10 excess vs mkt EW)'
            else:
                cum_path = f'{save_path}/group_performance_{ret_col.lower()}.png'
                mean_path = f'{save_path}/group_mean_{ret_col.lower()}.png'
                plot_title = f'{fname_prefix} — {ret_col} (G1-G10 excess vs mkt EW)'

            os.makedirs(save_path, exist_ok=True)
            save_decile_hl_cumsum_png(
                pnl,
                title=plot_title,
                out_path=cum_path,
                rank_ic=rank_ic,
                icir=icir,
                avg_turnover=avg_turnover_b,
            )
            save_decile_hl_mean_bar_png(
                pnl,
                title=f'{fname_prefix} — {ret_col} group mean excess vs mkt EW',
                out_path=mean_path,
            )
            print(f"✓ 十分组 H-L 图已保存: {cum_path}")
            print(f"✓ 分组均值柱状图已保存: {mean_path}")

            if show_plots:
                try:
                    plt.show()
                except Exception as e:
                    print(f"注意: 当前环境无法显示图片 ({e})")

        print(f"\n{ret_col} - 性能指标汇总:")
        print("-" * 80)
        for bartime in bartimes:
            print(f"\nBartime: {pd.to_datetime(bartime).strftime('%H:%M')}")
            ...
            if bartime in results_summary[ret_col]:
                for group in groups:
                    if group in results_summary[ret_col][bartime]:
                        ...
                        label = _group_display_name(group)
                        print(
                            f"{label:<12} {metrics['sharpe']:<15.3f} "
                            f"{metrics['max_drawdown']:<15.2%} "
                            f"{metrics['annualized_return']:<18.2%}"
                        )
...
def subtract_market_return(group_data_ret):
    """从分组收益中减去等权市场均值，使曲线基于超额收益。

    ``group_data_ret`` 来自 ``get_cs_group_performance``，典型列：
    ``group``, ``Date``, ``Bartime``, ``Ret_*``。

    市场均值仅用分位数组（``group_0``…，十分组时为 G1–G10）在每个
    (Date, Bartime) 截面上等权平均；``group_HML`` 本身已是多空差，保持不变
    （H−L = (H−m)−(L−m)）。
    """
    df = group_data_ret.copy()
    ret_cols = [c for c in df.columns if str(c).startswith("Ret_")]
    if not ret_cols:
        return df

    date_col = "Date" if "Date" in df.columns else None
    if date_col is None and "tradetime" in df.columns:
        date_col = "tradetime"
    if date_col is None or "Bartime" not in df.columns:
        raise KeyError(
            "subtract_market_return needs Date (or tradetime) and Bartime columns; "
            f"got {list(df.columns)}"
        )

    group_col = "group" if "group" in df.columns else ("Group" if "Group" in df.columns else None)
    if group_col is None:
        # no group column — demean all rows by Date×Bartime
        market = df.groupby([date_col, "Bartime"], sort=False)[ret_cols].transform("mean")
        for col in ret_cols:
            df[col] = df[col] - market[col]
        return df

    is_hml = df[group_col].astype(str).str.contains("HML", case=False, na=False)
    base = df.loc[~is_hml]
    if base.empty:
        return df

    mkt_stats = (
        base.groupby([date_col, "Bartime"], sort=False)[ret_cols]
        .mean()
        .reset_index()
    )
    mkt_full = df[[date_col, "Bartime"]].merge(
        mkt_stats, on=[date_col, "Bartime"], how="left"
    )
    for col in ret_cols:
        demeaned = df[col] - mkt_full[col]
        df.loc[~is_hml, col] = demeaned.loc[~is_hml]
    return df