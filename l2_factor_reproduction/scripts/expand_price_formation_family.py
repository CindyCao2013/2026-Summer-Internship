#!/usr/bin/env python
"""Run the frozen daily baseline for Price Formation Family v1."""
...
def _cross_family_summary() -> Tuple[str, str]:
    frames = []
    for family in ("trade_flow", "order_size", "order_book"):
        path = POOL_DIR / f"price_formation_vs_{family}_corr.csv"
        if path.exists():
    ...
    maxima = (
        cross.groupby("price_formation_factor")[
    ...
    text = (
        f"各 Price Formation 代表对既有三族参考的最大 |ρ| 中位数="
...
def _write_report(summary: pd.DataFrame) -> None:
    ...
    lines = [
        "# L2 Candidate Pool v1 — Price Formation Family",
        "",
        "范围：冻结 level 公式 baseline discovery、年度稳定性与只读冗余"
        "证据。未做参数搜索、周/双周优化、二次中性化、组合、机器学习或 "
        "KEEP/DROP。",
        "",
        "## Research accounting",
        "",
        ...
        f"- Eligible rows：{int(primitive_manifest['eligible_row_count']):,}",
        f"- Coverage mean："
        f"{float(primitive_manifest['coverage_statistics']['mean']):.2%}",
        "- Continuous grid：09:30–11:29、13:00–14:59；15:00 单独；"
        "午间不 forward fill；价格状态最多填 3 分钟，amount/volume 不填。",
        "- ClickHouse KLIN 只用于 2024-06 parity；没有静默拼接。",
        "- active-flow 33–35 因两市 KLIN 无一致定义而未加入。",
        "",
        "## Unified baseline",
        "",
...
def main() -> int:
    ...
    for position, name in enumerate(names, start=1):
        ...
        spec = PRICE_FORMATION_FACTOR_SPECS[name]
        row = {
            "factor": name,
    ...
    manifest = {
        "version": "price_formation_candidate_pool_v1",
        ...
        "scope_exclusions": [
            ...
            "factor_combination",
            "machine_learning",
            "weekly_or_biweekly_optimization",
            "second_neutralization",
            "keep_drop",
            "active_flow_with_inconsistent_exchange_definition",
        ],
    }
