#!/usr/bin/env python
"""Single-Factor Full Validation v1 — effective_spread_proxy.

Frozen formula (candidate-pool / Fast Discovery verified; NOT modified):
    effective_spread_proxy from liquidity_impact_daily
      = mean(2*sign(signed_amount)*(trade_vwap-midquote)/midquote)
    signal shift = T+1
    Minute approximation; NOT per-trade prevailing-quote effective spread.

Usage:
    python -m l2_factor_reproduction.scripts.validate_effective_spread_proxy
"""
...
FACTOR = "effective_spread_proxy"
FORMULA_EXACT = (
    "effective_spread_proxy = mean(2*sign(signed_amount)*(trade_vwap-midquote)/midquote); "
    "minute approximation over continuous auction minutes; "
    "signed_amount = active_buy_amount - active_sell_amount; "
    "midquote = (bid1+ask1)/2; signal_shift = T+1; "
    "NOT per-trade prevailing-quote effective spread"
)
FORMULA_HASH = "1a0577314b7e974272fb1a98007cb4c6a0b5486a62f02601f1db9e673c83bbf2"
...
def build_factor_narrow(start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    liq = load_primitive_panel(
        ...
        ["effective_spread_proxy", "coverage_ratio"],
        start,
        end,
        buffer_days=5,
    )
    ...
    return _to_narrow(
        ...
        liq.loc[mask, "effective_spread_proxy"],
        FACTOR,
    )
...
def render_report(
    ...
    lines = [
        ...
        "",
        "## Frozen formula (candidate-pool / Fast Discovery)",
        "",
        "```",
        ...
        "```",
        "",
        "Proxy note: minute approximation; NOT per-trade prevailing-quote effective spread.",
        f"formula_hash_sha256 = `{FORMULA_HASH}`",
        "",
...
def main() -> int:
    ...
    manifest = {
        ...
        "formula_frozen": {
            "exact_formula": FORMULA_EXACT,
            "formula_hash_sha256": FORMULA_HASH,
