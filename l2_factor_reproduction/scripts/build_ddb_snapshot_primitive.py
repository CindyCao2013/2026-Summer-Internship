#!/usr/bin/env python
"""Build the partitioned ddb_reference_snapshot daily primitive dataset.

Chunks are quarter-partitioned parquet files; each quarter is fetched as
monthly ClickHouse queries (memory bound) and concatenated.  All factor
computation is pushed down to ClickHouse (ch_ddb_snapshot.ddb_snapshot_daily_sql).

Usage:
    /opt/conda/anaconda3/envs/base_93/bin/python \
        l2_factor_reproduction/scripts/build_ddb_snapshot_primitive.py \
        --start 2024-06-28 --end 2024-06-28 --chunk-frequency month
"""
...
def _chunk_path(start: pd.Timestamp, end: pd.Timestamp) -> Path:
    ...
    return year_dir / (
        f"ddb_snapshot_daily_{start:%Y-%m-%d}_{end:%Y-%m-%d}.parquet"
    )
...
def _audit_chunk(
    ...
    for name in FACTOR_NAMES:
        ...
        audit[f"{name}_null_rows"] = int(frame[mean_col].isna().sum())
        audit[f"{name}_abs_q50"] = float(values.abs().quantile(0.50))
        audit[f"{name}_abs_q99"] = float(values.abs().quantile(0.99))
        audit[f"{name}_abs_q999"] = float(values.abs().quantile(0.999))
        audit[f"{name}_abs_max"] = float(values.abs().max())
        audit[f"{name}_inf_rows"] = int(
            frame[f"{name}_inf_count"].sum()
        )
        audit[f"{name}_clipped_rows_total"] = int(
            frame[f"{name}_clipped_count"].sum()
        )
        audit[f"{name}_small_den_total"] = int(
            frame[f"{name}_small_denominator_count"].sum()
        )
        audit[f"{name}_ffill_total"] = int(
            frame[f"{name}_ffill_count"].sum()
        )
        audit[f"{name}_null_snapshot_total"] = int(
...
def _assert_hard_checks(audit: Dict[str, object]) -> None:
    hard = ("duplicate_keys", "coverage_above_one", "inf_values")
    failures = {name: audit[name] for name in hard if audit[name] != 0}
    for name in FACTOR_NAMES:
        if audit[f"{name}_inf_rows"] != 0:
            failures[f"{name}_inf_rows"] = audit[f"{name}_inf_rows"]
    if failures:
        raise ValueError(
            f"DDB snapshot primitive chunk failed hard checks: {failures}"
        )
...
def _query_hashes(start: pd.Timestamp, end: pd.Timestamp) -> Dict[str, str]:
    hashes = {}
    for table, suffix, exchange in DDB_SNAPSHOT_TABLES:
...
def _write_metadata(
    *,
    ...
    code_path = (
        PROJ_ROOT / "l2_factor_reproduction/python/ch_ddb_snapshot.py"
    )
    manifest = {
        "primitive_name": "l2_primitive_ddb_reference_snapshot_daily",
        "schema_version": SCHEMA_VERSION,
        "formula_version": FORMULA_VERSION,
        "source_tables": [
            f"cmds.{table}" for table, _, _ in DDB_SNAPSHOT_TABLES
        ],
    ...
    query_metadata = {
        ...
        "formula_version": FORMULA_VERSION,
        "query_builder": (
            "l2_factor_reproduction.python.ch_ddb_snapshot."
            "ddb_snapshot_daily_sql"
        ),
        "minute_sampling": "argMax(factor, ExchTime)",
        "daily_aggregate": "mean over 240-minute fixed grid",
        "sessions": ["[09:30,11:30)", "[13:00,15:00)"],
        "close_auction": "15:00 row joins state evolution, excluded from mean",
        "chunk_query_hashes": {
...
def main() -> int:
    ...
    for index, (chunk_start, chunk_end) in enumerate(planned, start=1):
        ...
        if path.exists() and not args.overwrite:
            ...
            # bound each CH query to one day inside the chunk:
            # month-wide HTTP queries exceed the 300s read timeout
            month_frames: List[pd.DataFrame] = []
            for m_start, m_end in _chunks(chunk_start, chunk_end, "month"):
                print(
                    f"  [month] {m_start:%Y-%m-%d}..{m_end:%Y-%m-%d}",
                    flush=True,
                )
                day_frames: List[pd.DataFrame] = []
                for d_start, d_end in _chunks(m_start, m_end, "day"):
                    day_frame = fetch_ddb_snapshot_daily(d_start, d_end)
            ...
            frame = pd.concat(month_frames, ignore_index=True)
            from l2_factor_reproduction.python.ch_ddb_snapshot import (
                prepare_ddb_snapshot_daily,
            )

            frame = prepare_ddb_snapshot_daily(frame)
        ...
        coverage_rows.extend(_coverage_rows(frame, chunk_start, chunk_end))
