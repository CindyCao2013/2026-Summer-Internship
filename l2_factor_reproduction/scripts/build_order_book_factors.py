#!/usr/bin/env python
"""Stream the partitioned Order Book primitive into 32 frozen narrows."""
...
def _primitive_files(
    ...
    files = sorted(DATASET_DIR.glob("year=*/order_book_daily_*.parquet"))
    selected = []
    for path in files:
        parts = path.stem.removeprefix("order_book_daily_").split("_")
        if len(parts) != 2:
            continue
    ...
    if not selected:
        raise FileNotFoundError(
            f"No primitive partitions under {DATASET_DIR}"
        )
    return selected
...
def _write_heatmap(corr: pd.DataFrame) -> None:
    ...
    ax.set_title("Order Book Family — mean daily cross-sectional Spearman")
...
def main() -> int:
    ...
    try:
        for position, path in enumerate(files, start=1):
            ...
            primitive = pd.read_parquet(
                path,
                columns=list(REQUIRED_PRIMITIVE_COLUMNS),
            )
            ...
            if primitive.empty:
                continue
            features, history = build_order_book_feature_chunk(
                primitive, history
            )
    ...
    if not success:
        raise RuntimeError("Factor build did not complete")
    ...
    manifest = {
        "version": "order_book_factor_layer_v1",
        ...
        "rolling_shocks": (
            "inclusive current plus up to 19 historical observations; "
            "no future rows"
        ),
        "correlation_method": (
