#!/usr/bin/env python
"""Single-Factor Full Validation v2.0 — tail_return_share.

Inventory reuse only. No formula / parameter / threshold edits.
Does not validate intraday_max_drawdown or close_auction_return.
Does not modify Sprint 11 artifacts.

Usage:
    python -m l2_factor_reproduction.scripts.validate_tail_return_share
"""
...
FACTOR = "tail_return_share"
CLUSTER = "tail_heaviness"
NEAR_ALIAS = "realized_kurtosis"

EXACT_FORMULA = (
    "tail_return_share = sum(r_t^2 where abs(r_t) >= daily_q95) / sum(r_t^2); "
    "r_t = minute return within continuous-auction session from "
    "price_formation_daily; daily_q95 = same-day quantile(abs(r_t), 0.95); "
    "TAIL_QUANTILE=0.95; signal_shift = T+1; no rolling transform; "
    "no parameter grid; source = price_formation_daily.tail_return_share; "
    "formula_version = price_formation_level_formulas_v1"
)
FACTOR_HASH = hashlib.sha256(EXACT_FORMULA.encode("utf-8")).hexdigest()
SOURCE_PRIMITIVE = "price_formation_daily.tail_return_share"
FORMULA_VERSION = "price_formation_level_formulas_v1"
SCHEMA_VERSION = "l2_primitive_price_formation_daily_v1"
EXPECTED_RAW_DIRECTION = -1  # from candidate_pool full-sample summary

DISCOVERY_METRICS = {
    "hl_sharpe": 3.88,
    "mono": 1.00,
    "violations": 0,
    "g10_gross_excess_annual": 0.217,
    "long_contribution_annual": 0.217,
    "short_contribution_annual": 0.332,
}
...
def build_factor_freeze() -> Dict[str, Any]:
    return {
        ...
        "redundancy_metadata": {
            ...
            "note": (
                "Sprint 11 within-family: |discovery Spearman|>=0.90 with "
                "realized_kurtosis; realized_kurtosis is not an independent alpha. "
                "Metadata only — does not alter this Full Validation."
            ),
        },
        "evidence_labels": {
            "DISCOVERY": {
        ...
        "exclusions": [
            "do_not_validate_intraday_max_drawdown",
            "do_not_test_close_auction_return",
            "do_not_modify_sprint11",
            "do_not_combine_with_realized_kurtosis",
            "do_not_retune_tail_threshold",
        ],
    }
    ...
    *,
    ...
    return {
        ...
        "dominant_leg": (
            ...
            else ("long" if abs(long_annu) > abs(short_annu) * 1.25 else "mixed")
        ),
    }
...
def render_report(
    ...
    lines = [
        "# Single-Factor Full Validation v2.0 — `tail_return_share`",
        "",
        "Protocol v2.0 FINAL FROZEN. Sprint 11 untouched. No formula edits.",
        "",
        ...
        "",
        "This family already had FULL history computed; FULL/POST are **not** described as pristine unseen test sets.",
        "",
        "## Factor freeze",
        "",
        f"- factor_id: `{FACTOR}`",
        f"- exact_formula: `{EXACT_FORMULA}`",
        f"- factor_hash_sha256: `{FACTOR_HASH}`",
        ...
        "",
        "## Redundancy metadata (Sprint 11; does not alter FV)",
        "",
        f"- cluster: `{CLUSTER}`",
        f"- representative: `{FACTOR}`",
        f"- near-alias: `{NEAR_ALIAS}` (|discovery Spearman|>=0.90)",
        f"- `{NEAR_ALIAS}` is **not** counted as independent alpha",
        "",
        "## Sprint 11 discovery snapshot (frozen record)",
        "",
        ...
        "",
        "| sample | RankIC | ICIR | gross H-L ann | gross Sharpe | net H-L ann | net Sharpe | mono | viol | pos-month |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines += [
        "",
        f"- Discovery legs (this FV recompute): long={_fmt_pct(leg_disc['long_contribution'])}, "
        f"short={_fmt_pct(leg_disc['short_contribution'])} "
        f"(Sprint 11 record ≈ long 21.7% / short 33.2%)",
        ...
        "",
        "## PART L — Final answers",
        "",
        f"1. Discovery Sharpe 3.88 → FULL gross H-L Sharpe = **{_fmt_num(full['gross_hl_sharpe'], 2)}** "
        f"(ann={_fmt_pct(full['gross_hl_annual'])}).",
        f"2. FULL mono/viol = **{_fmt_num(full['decile_mono_gross'], 3)} / {int(full['adjacent_violations_gross'])}** "
        f"→ {'YES' if float(full['decile_mono_gross']) >= 0.80 and int(full['adjacent_violations_gross']) <= 2 else 'NO'} "
        f"(mono>=0.80 & viol<=2).",
        f"3. POST RankIC={_fmt_num(post['rank_ic'], 4)}, Sharpe={_fmt_num(post['gross_hl_sharpe'], 2)}, "
        f"mono={_fmt_num(post['decile_mono_gross'], 3)}; "
        f"IC_ret={retention['IC_retention']:.3f}, Sharpe_ret={retention['Sharpe_retention']:.3f}; "
        f"flags=`{', '.join(flags) if flags else 'none'}`.",
        f"4. FULL net H-L annual/Sharpe = **{_fmt_pct(full['net_hl_annual'])} / {_fmt_num(full['net_hl_sharpe'], 2)}** "
        f"(fee={_fmt_pct(full['fee_annualized'])}).",
        f"5. Factor Grade = **{factor_grade}**.",
        f"6. Long-only FULL: net excess={_fmt_pct(long_full['excess_annual_return'])}, "
        f"IR={_fmt_num(long_full['IR'], 2)}, "
        f"pos-month={_fmt_pct(long_full['positive_excess_month_fraction'])}.",
        f"7. Long-only Status = **{long_only_status}**.",
        f"8. Alpha mix — FULL: **{leg_full['dominant_leg']}** "
        f"(long={_fmt_pct(leg_full['long_contribution'])}, short={_fmt_pct(leg_full['short_contribution'])}); "
        f"POST: **{leg_post['dominant_leg']}** "
        f"(long={_fmt_pct(leg_post['long_contribution'])}, short={_fmt_pct(leg_post['short_contribution'])}).",
        f"9. Keep as `{CLUSTER}` representative? "
        + (
            "**YES** — still strongest validated non-ordinary tail-heaviness signal; "
            f"`{NEAR_ALIAS}` remains near-alias, not independent."
            if factor_grade.startswith(("A", "B"))
            else f"**CONDITIONAL** — grade `{factor_grade}`; retain as cluster label only if research continues."
        ),
        f"10. Final freeze status = **{freeze_status}** "
        f"(factor_grade=`{factor_grade}`, long_only=`{long_only_status}`).",
        "",
        "## STOP",
        "",
        "No auto-validation of other factors. No threshold / definition / lookback edits.",
        "",
    ]
    return "\n".join(lines)
...
def main() -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "raw_direction": direction,
        "cache_reuse": cache,
        "redundancy_metadata": freeze["redundancy_metadata"],
        ...
        "plots": plot_paths,
        "sprint11_untouched": True,
        "validated_others": False,
