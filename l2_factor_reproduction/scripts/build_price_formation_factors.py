#!/usr/bin/env python
"""Stream Price Formation primitive partitions into 32 frozen factors."""
...
PRIMITIVE_DIR = Path(RESULT_ROOT) / "primitives" / "price_formation_daily"
DATASET_DIR = PRIMITIVE_DIR / "dataset"
POOL_DIR = Path(RESULT_ROOT) / "candidate_pool_v1" / "price_formation_family"
...
def _parse_names(value: str) -> List[str]:
    if value.strip().lower() in {"", "all"}:
        return list(PRICE_FORMATION_FACTOR_NAMES)
    names = [item.strip() for item in value.split(",") if item.strip()]
    unknown = sorted(
        set(names).difference(PRICE_FORMATION_FACTOR_SPECS)
    )
    if unknown:
        raise ValueError(f"Unknown Price Formation factors: {unknown}")
    if not names:
        raise ValueError("Factor list is empty")
    return names


def _primitive_files(
    start: pd.Timestamp,
    end: pd.Timestamp,
) -> List[Path]:
    files = sorted(
        DATASET_DIR.glob("year=*/price_formation_daily_*.parquet")
    )
    selected = []
    for path in files:
        parts = path.stem.replace(
            "price_formation_daily_", "", 1
        ).split("_")
        if len(parts) != 2:
            continue
        chunk_start, chunk_end = map(pd.Timestamp, parts)
        if chunk_end >= start and chunk_start <= end:
            selected.append(path)
    if not selected:
        raise FileNotFoundError(
            f"No Price Formation primitive partitions under {DATASET_DIR}"
        )
    return selected
...
def _write_heatmap(corr: pd.DataFrame) -> None:
    ...
    ax.set_title(
        "Price Formation Family — mean daily cross-sectional Spearman"
    )
...
def _write_aliases(
    ...
    rows = []
    for name in names:
        exclusion = PRICE_FORMATION_FACTOR_SPECS[name].alias_exclusions
        if exclusion:
            rows.append(
                {
                    "factor": name,
                    "alias_type": "formula_exclusion",
                    "peer_or_clone": exclusion,
                    "abs_mean_daily_spearman": np.nan,
                    "counted_as_candidate": False,
                }
            )
    empirical = pairs.loc[
        pairs["abs_mean_daily_spearman"] >= 0.95
    ]
    for row in empirical.itertuples(index=False):
        rows.append(
            {
                "factor": row.factor_left,
                "alias_type": "empirical_near_alias",
                "peer_or_clone": row.factor_right,
                "abs_mean_daily_spearman": row.abs_mean_daily_spearman,
                "counted_as_candidate": True,
            }
        )
    pd.DataFrame(
        rows,
        columns=[
            ...
            "counted_as_candidate",
        ],
    ).to_csv(POOL_DIR / "aliases.csv", index=False)
...
def main() -> int:
    ...
    writers, temporary_paths = _open_writers(
        names, overwrite=args.overwrite
    )
    coverage = {
        name: {
            "factor": name,
    ...
    corr_count = pd.DataFrame(0, index=names, columns=names, dtype=np.int64)
    ...
    try:
        for position, path in enumerate(files, start=1):
            ...
            if primitive.empty:
                continue
            features, history = build_price_formation_feature_chunk(
                primitive, history
            )
    ...
    if not success:
        raise RuntimeError("Price Formation factor build did not complete")
    ...
    if not args.skip_correlation:
        ...
        pairs.to_csv(
            POOL_DIR / "factor_correlation_pairs.csv", index=False
        )
        pairs.loc[
    ...
    manifest = {
        "version": "price_formation_factor_layer_v1",
        ...
        "direction_policy": (
            "raw formula direction frozen; effective direction is display-only"
        ),
    }
    ...
    print(
        f"[done] factors={len(names)} -> {FACTOR_ROOT}",
        flush=True,
    )
    return 0
