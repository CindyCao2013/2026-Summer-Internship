#!/usr/bin/env python
"""Prefetch IdealAmplitude_ActiveV2 daily bricks, then run Factor_Test_Process.

Usage:
  OMP_NUM_THREADS=1 python run_ideal_amplitude_active_v2_backtest.py
  OMP_NUM_THREADS=1 python run_ideal_amplitude_active_v2_backtest.py --prefetch-only
"""
...
from core.l2_features.ideal_amplitude_active_v2_builder import ensure_daily_bricks
...
def main() -> None:
    ...
    print(
        f"=== Prefetch IdealAmplitude daily bricks {start.date()}..{end.date()} "
        f"(track={cfg.TRACK}) ===",
        flush=True,
    )
