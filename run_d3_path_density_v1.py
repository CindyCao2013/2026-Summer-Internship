#!/usr/bin/env python
"""D3 Price Path / Support Alpha Density Mining v1.

Internal family density + cross-dimension orthogonal IC (incl. D4 conditional IC)
+ stack λ-grid on D1+D4+D5 base + D3/D4 synergy check.

Usage:
  OMP_NUM_THREADS=1 python run_d3_path_density_v1.py
  OMP_NUM_THREADS=1 python run_d3_path_density_v1.py --no-reports
"""
...
from alpha_dimension_density import (
    ...
    conditional_ic_by_anchor_tercile,
...
from factor_formulas_path_d3 import D3_PATH_DENSITY_CANDIDATES, D3_REPRESENTATIVE
...
OUT = Path("research/reports/d3_path_density_v1")


def main() -> None:
    parser = argparse.ArgumentParser(description="D3 Path Alpha Density v1")
    parser.add_argument("--discovery-days", type=int, default=DISCOVERY_DAYS)
    parser.add_argument("--no-reports", action="store_true")
    args = parser.parse_args()

    OUT.mkdir(parents=True, exist_ok=True)
    log("=== D3 Price Path / Support Alpha Density v1 ===")
    ...
    d3_rep = ctx["frozen_panels"][D3_REPRESENTATIVE]
    d4_rep = ctx["frozen_panels"]["winner_sentiment_reversal_5d"]
    tri_base = build_tri_base_stack(ctx["frozen_panels"])

    density_rows = []
    report_summaries = []

    for name, source, hypothesis, family in D3_PATH_DENSITY_CANDIDATES:
        log(f"\n--- {name} ({source}, {family}) ---")
        try:
            ...
            for period_label, ret_w in [("discovery", ret_disc), ("confirmation", ret_conf)]:
                ...
                if period_label == "discovery":
                    log(
                        f"  [{period_label}] IC={row['ic_raw']:.4f} | after D3={row['ic_after_d3']:.4f} | "
    ...
    for period_label, ret_w in [("discovery", ret_disc), ("confirmation", ret_conf)]:
        if len(ret_w) < 60:
            continue
        cond = conditional_ic_by_anchor_tercile(
            d3_rep.reindex(index=ret_w.index, columns=ret_w.columns),
            ret_w,
            d4_rep.reindex(index=ret_w.index, columns=ret_w.columns),
            anchor_label="D4_rep",
        )
        cond["period"] = period_label
        conditional_rows.append(cond)
        log(f"\n=== D3 conditional IC within D4 terciles ({period_label}) ===")
        log(cond.to_string(index=False, float_format=lambda x: f"{x:.4f}"))
    ...
    conditional_df = pd.concat(conditional_rows, ignore_index=True) if conditional_rows else pd.DataFrame()

    synergy = {}
    if len(ret_disc) >= 60:
        quad_no_d3 = build_quad_base_without(ctx["frozen_panels"], "D3").reindex(
            index=ret_disc.index, columns=ret_disc.columns
        )
        full_five = combine_equal_weight(
            [ctx["frozen_panels"][s["factor"]] for s in FROZEN_OHLCV_REPS]
        ).reindex(index=ret_disc.index, columns=ret_disc.columns)
        tri_d4 = combine_equal_weight(
            [
                ctx["frozen_panels"]["low_vol_liquidity_quality_60d"],
                ctx["frozen_panels"]["winner_sentiment_reversal_5d"],
                ctx["frozen_panels"]["upside_fragility_20d"],
            ]
        ).reindex(index=ret_disc.index, columns=ret_disc.columns)
        d3_d4_pair = combine_equal_weight([d3_rep, d4_rep]).reindex(
            index=ret_disc.index, columns=ret_disc.columns
        )
        _, _, delta_full = compare_stacks(quad_no_d3, full_five, ret_disc)
        _, _, delta_d4_only = compare_stacks(tri_d4, combine_equal_weight([tri_d4, d3_rep]).reindex(
            index=ret_disc.index, columns=ret_disc.columns
        ), ret_disc)
        _, _, delta_d3d4 = compare_stacks(d4_rep.reindex(index=ret_disc.index, columns=ret_disc.columns), d3_d4_pair, ret_disc)
        synergy = {
            "discovery_quad_vs_full5": delta_full,
            "discovery_tri_d4_plus_d3": delta_d4_only,
            "discovery_d4_vs_d3_d4_pair": delta_d3d4,
        }
    ...
    verdict = summarize_dimension_verdict(
        ...
        stack_disc,
        stack_conf,
        extra={"synergy_tests_discovery": synergy},
    )
