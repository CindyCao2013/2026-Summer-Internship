#!/usr/bin/env python3
"""Render the TGD20 Markdown report as a self-contained HTML memo."""
...
ROOT = next(
    ...
    if (parent / "research_delivery/TGD20_research_package").is_dir()
)
REPORT_DIR = ROOT / "research_delivery/TGD20_research_package/report"
SOURCE = REPORT_DIR / "TGD20_买方因子研究报告.md"
TARGET = REPORT_DIR / "TGD20_买方因子研究报告.html"
...
def extract_math(text: str):
    ...
    def replace(match):
        token = f"TGD20MATHPLACEHOLDER_{len(replacements)}_END"
        replacements[token] = (
