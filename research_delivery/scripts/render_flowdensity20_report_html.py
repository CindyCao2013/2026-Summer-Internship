#!/usr/bin/env python3
"""Render the FlowDensity20 Markdown report as a self-contained HTML memo."""
...
ROOT = next(
    ...
    if (parent / "research_delivery/FlowDensity20_research_package").is_dir()
)
REPORT_DIR = ROOT / "research_delivery/FlowDensity20_research_package/report"
SOURCE = REPORT_DIR / "FlowDensity20_买方因子研究报告.md"
TARGET = REPORT_DIR / "FlowDensity20_买方因子研究报告.html"
...
def expand_tables(text: str) -> str:
    """Convert GitHub-style tables for the legacy Mistune 0.8 renderer."""
...
def extract_math(text: str):
    """Protect LaTeX from Markdown and return token replacements."""
    replacements = {}
    pattern = re.compile(r"\\\[(.+?)\\\]|\\\((.+?)\\\)", re.DOTALL)

    def replace(match):
        token = f"FLOWDENSITY20MATHPLACEHOLDER_{len(replacements)}_END"
        replacements[token] = (
...
def render_math(expression: str, display: bool) -> str:
    """Render LaTeX with Matplotlib mathtext as an embedded SVG."""
    output = io.BytesIO()
    math_to_image(
        f"${expression}$",
        output,
...
def inline_report_images(body: str) -> str:
    """Embed local report images so the HTML works as a standalone file."""
...
<title>FlowDensity20 买方单因子研究报告</title>
<style>{CSS}</style>
</head>
