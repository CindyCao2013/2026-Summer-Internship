#!/usr/bin/env python3
"""Production trace plus gated signal/backtest parity for the bar-level proxy."""
...
FLAG = "INTRADAY_LARGE_ACTIVE_BUY_RATIO_USE_DDB"
PARITY_ROOT = ROOT / "result" / "intraday" / "_parity_large_active_buy_ratio"
SAMPLE_START = "2024-05-01"
SAMPLE_END = "2024-05-31"


def trace_production_path() -> None:
    """Prove registry dispatch reaches discovery-v1 DDB without a live query."""
    ...
    with unittest.mock.patch.object(
        discovery_v1, "ddb_version", return_value=fake
    ) as mock_ddb:
...
def run_signal_parity(start: str, end: str) -> Dict[str, Any]:
    from factors.intraday.large_active_buy_ratio.test_compare import (
        assert_consistency,
    )
