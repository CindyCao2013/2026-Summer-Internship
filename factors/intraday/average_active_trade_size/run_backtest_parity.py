#!/usr/bin/env python3
"""Production, signal, and backtest parity for average_active_trade_size."""
...
PARITY_ROOT = ROOT / "result" / "intraday" / "_parity_average_active_trade_size"
SAMPLE_START = "2024-05-01"
SAMPLE_END = "2024-05-31"


def trace_production_path() -> None:
    """Prove registry dispatch reaches the shared DDB implementation."""
...
def _build_signal(use_ddb: bool, start: str, end: str) -> pd.DataFrame:
    ...
    with unittest.mock.patch.dict(
        ...
        return build_intraday_narrow_table(
            FACTOR_NAME, start, end, store=store
        )


def run_signal_parity(start: str, end: str) -> Dict[str, Any]:
    from factors.intraday.average_active_trade_size.test_compare import (
        assert_consistency,
    )
