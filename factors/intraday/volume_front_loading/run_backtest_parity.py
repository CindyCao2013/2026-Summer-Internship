#!/usr/bin/env python3
"""Production-path, signal and full-backtest parity for volume_front_loading.

This runner consumes the existing Intraday_Factor_Test_Process.py unchanged.
"""
...
FACTOR = "volume_front_loading"
FLAG = "INTRADAY_VOLUME_FRONT_USE_DDB"
PARITY_ROOT = ROOT / "result" / "intraday" / "_parity_volume_front_loading"
...
def trace_production_path() -> None:
    ...
    print(
        ...
        "compute_volume_front_loading → ddb_version",
        flush=True,
    )
...
def run_signal_parity(start: str, end: str) -> Dict[str, Any]:
    from factors.intraday.volume_front_loading.test_compare import (
        assert_consistency,
    )
...
s = namespace["s"]
ic_mean = pd.DataFrame(s.run("summary['ic_mean']"))
ic_mean.to_csv(dst / "ic_mean.csv", index=False)
...
"""
    ...
    env = os.environ.copy()
    env[FLAG] = "true" if use_ddb else "false"
...
def _compare_ic(py_dir: Path, db_dir: Path) -> Dict[str, float]:
    ...
    if not (joined["_merge"] == "both").all():
        raise AssertionError("IC key mismatch between Python and DDB runs")
    return {
...
def _compare_performance(py_dir: Path, db_dir: Path) -> Dict[str, float]:
    ...
    hl = joined["Group"] == "group_HML"
    decile = joined["Group"].str.match(r"group_\d+")
    return {
        "excess_decile_sharpe_max_abs_diff": float(diff[decile].max()),
        "hl_sharpe_max_abs_diff": float(diff[hl].max()),
    }
...
def compare_backtests(py_dir: Path, db_dir: Path) -> Dict[str, Any]:
    ...
    if report["signal_count_diff"] != 0 or report["universe_count_diff"] != 0:
